...
- (id)init
{
    if ((self = [super init]))
    {
        ...
        diskCachePath = SDWIReturnRetained([[paths objectAtIndex:0] stringByAppendingPathComponent:@"ZZCaches/ImageCache"]);
...
//        [memCache setObject:image forKey:key cost:image.size.height * image.size.width * image.scale];
    ...
    {
        if ([[NSUserDefaults standardUserDefaults] boolForKey:ISNOIMAGE])//是否开启2G/3G无图模式
        {
